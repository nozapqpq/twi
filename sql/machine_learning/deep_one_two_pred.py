...
class DeepOneTwoPred():
    ...
    def get_dl_element10(self, dct):
        zi_diff = min(self.sr.single_horse_dicts[dct["today_horsenum"]-1]["zi_diff_from_top"],30)/30
        return zi_diff
    def get_dl_element11(self, dct):
        return min(max(dct["past_time_diff"],-2.5),2.5)
    ...
    def get_dl_element85(self, dct):
        # 10.0sを0, 13.0sを1とする
        return (min(max(dct["past_early_rap2"],10.0),13.0)-10.0)/3.0
    def get_dl_element86(self, dct):
        return (min(max(dct["past_early_rap3"],10.0),13.0)-10.0)/3.0
    def get_dl_element87(self, dct):
        return (min(max(dct["past_early_rap4"],10.0),13.0)-10.0)/3.0
    def get_dl_element88(self, dct):
        return (min(max(dct["past_last_rap1"],10.0),13.0)-10.0)/3.0
    def get_dl_element89(self, dct):
        return (min(max(dct["past_last_rap2"],10.0),13.0)-10.0)/3.0
    def get_dl_element90(self, dct):
        return (min(max(dct["past_last_rap3"],10.0),13.0)-10.0)/3.0
    def get_dl_element91(self, dct):
        return (min(max(dct["past_last_rap4"],10.0),13.0)-10.0)/3.0
    def get_dl_element92(self, dct):
        return min(dct["past_diff3f"],2.5)/2.5
    def get_dl_element93(self, dct):
        return (min(dct["today_horsenum"],18)-1)/17
    def get_dl_element94(self, dct):
        return min(math.log(dct["past_odds"],3),5)/5
    def get_dl_element95(self, dct):
        if dct["today_span"] <= 0:
            return 0
        else:
            return min(math.log(dct["today_span"],2),8)/6
    def get_dl_element96(self, dct):
        return min(math.log((dct["today_rdate"]-dct["past_rdate"]).days,4),4)/4
