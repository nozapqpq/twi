...
class AnalyseResult():
    ...
    def analyse_deeplearning_output(self, out_list, today_date):
        ...
        with open("../deeplearning_research_result.csv","a") as f:
            ...
            writer = csv.writer(f)
            writer.writerow([today_date]+self.get_analyse_result(places,out_list,"1着度","over",4))
            # 着外度低い順に並べ替え
            out_list = sorted(out_list, reverse=False, key=lambda x: x[6])
            writer.writerow([today_date]+self.get_analyse_result(places,out_list,"着内度","under",6))
    ...
    def get_analyse_result(self, places, out_list, elem_name, direction, target):
        ...
        for p in places:
            for i in range(12):
                ...
                if single_analyse_list[0][9] > 0: # if there is any dividend, count
    ...
    def deep_learning(self, x_train, y_train, dim, horsename_list, pred_x_np, todayinfo_lst):
        ...
        history = model.fit(x_train, y_train, epochs=20, batch_size=2000, validation_split=0.1)
