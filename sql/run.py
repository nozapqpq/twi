...
entry, all_target = pat.get_entry_target_data("../today.csv")
jockey_csvout = []
...
for a in range(len(all_entry)):
    ...
    for ent in all_entry[a]:
        ...
        pat.maindata.append([self_data])
        pat.maindata.append(pat.get_finish_trend_list(pat.subdata))
        maindata_sum.append(pat.maindata)
        subdata_sum.append(pat.subdata)
        ent_count = ent_count + 1
    timediff_acc = pat.make_timediff_accumulation_list(maindata_sum,subdata_sum,all_entry,a)
